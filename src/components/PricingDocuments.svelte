<script lang="ts">
	...
	// put the current locale first
	docs.sort((a, b) => (a.lang === getLocale() ? -1 : b.lang === getLocale() ? 1 : 0));
</script>

<section class="mx-auto my-16 max-w-6xl px-4">
	<p class="text-lg md:text-xl">{m.price_pdf_desc()}</p>
	<div class="mt-4 flex gap-4">
		{#each docs as doc (doc.url)}
			<a
				href={doc.url}
				class="flex flex-col gap-1 rounded border border-neutral-200 bg-neutral-50 p-4 hover:bg-neutral-100"
				target="_blank"
